...
const GlassIcons = ({ items, className, onStart }) => {
  ...
  return (
    <div
      className={`flex gap-[5em] mx-auto py-[3em] overflow-visible cursor-pointer ${
        className || ''
      }`}
    >
      {items.map((item, index) => (
        ...
        >
          ...
          {/* Label */}
          <span className='absolute top-full left-0 right-0 text-center whitespace-nowrap leading-[2] opacity-0 transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] translate-y-0 group-hover:opacity-100 group-hover:[transform:translateY(20%)]'>
            {item.label}
          </span>
        </button>
      ))}
